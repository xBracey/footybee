...
describe("Test validateBoolean", () => {
  it("Valid(required) true", () => {
    const valid = validateBoolean(true, true);

    expect(valid).toBe(true);
  });

  it("Valid(required) false", () => {
    const valid = validateBoolean(false, true);

    expect(valid).toBe(true);
  });

  it("Invalid(required)", () => {
    const valid = validateBoolean(null, true);

    expect(valid).toBe(false);
  });

  it("Valid(not required)", () => {
    const valid = validateBoolean(null, false);

    expect(valid).toBe(true);
  });

  it("Invalid(not required)", () => {
    const valid = validateBoolean("true", false);
